...
public class KillSoundGoldKingZ : BasePlugin
{
    ...
    public override string ModuleAuthor => "Gold KingZ, +SyntX";
    ...
    public class PersonData
    {
        ...
        public float volume { get; set; } = 1.0f; // This includes volume
    }
    ...
    public HookResult OnEventPlayerConnectFull(EventPlayerConnectFull @event, GameEventInfo info)
    {
        ...
        if (!string.IsNullOrEmpty(Configs.GetConfigData().KS_OnlyAllowTheseGroupsToToggle) && 
            Helper.IsPlayerInGroupPermission(player, Configs.GetConfigData().KS_OnlyAllowTheseGroupsToToggle))
        {
            ...
            async Task PerformDatabaseOperationAsync()
            {
                try
                {
                    var connectionSettings = JsonConvert.DeserializeObject<MySqlDataManager.MySqlConnectionSettings>(
                        await File.ReadAllTextAsync(Path.Combine(ModuleDirectory, "config", "MySql_Settings.json")));
                    ...
                    using (var connection = new MySqlConnection(connectionString))
                    {
                        ...
                        var mysqlPersonData = await MySqlDataManager.RetrievePersonDataByIdAsync(playerid, connection);

                        KillSoundGoldKingZ.PersonData personData = new KillSoundGoldKingZ.PersonData
                        {
                            PlayerSteamID = mysqlPersonData.PlayerSteamID,
                            freezemenu = mysqlPersonData.freezemenu,
                            headshotkill = mysqlPersonData.headshotkill,
                            headshothit = mysqlPersonData.headshothit,
                            bodyshotkill = mysqlPersonData.bodyshotkill,
                            bodyshothit = mysqlPersonData.bodyshothit,
                            quakesounds = mysqlPersonData.quakesounds,
                            quakehmessages = mysqlPersonData.quakehmessages,
                            quakecmessages = mysqlPersonData.quakecmessages,
                            date = DateTime.Now,
                            volume = 1.0f
                        };
                        ...
                        {
                            Helper.SaveToJsonFile(playerid, personData.freezemenu, personData.headshotkill, 
                                personData.headshothit, personData.bodyshotkill, personData.bodyshothit, 
                                personData.quakesounds, personData.quakehmessages, personData.quakecmessages, 
                                personData.date);
                        }
                    }
                }
        ...
        {
            var helperPersonData = Helper.RetrievePersonDataById(playerid);
            
            // Convert Helper.PersonData to KillSoundGoldKingZ.PersonData
            KillSoundGoldKingZ.PersonData personData = new KillSoundGoldKingZ.PersonData
            {
                PlayerSteamID = helperPersonData.PlayerSteamID,
                freezemenu = helperPersonData.freezemenu,
                headshotkill = helperPersonData.headshotkill,
                headshothit = helperPersonData.headshothit,
                bodyshotkill = helperPersonData.bodyshotkill,
                bodyshothit = helperPersonData.bodyshothit,
                quakesounds = helperPersonData.quakesounds,
                quakehmessages = helperPersonData.quakehmessages,
                quakecmessages = helperPersonData.quakecmessages,
                date = DateTime.Now,
                volume = 1.0f
            };
    ...
    private HookResult OnPlayerDeath(EventPlayerDeath @event, GameEventInfo info)
    {
        ...
        // Convert Helper.PersonData to KillSoundGoldKingZ.PersonData
        var helperPersonData = Helper.RetrievePersonDataById(attackerid);
        KillSoundGoldKingZ.PersonData personData = new KillSoundGoldKingZ.PersonData
        {
            PlayerSteamID = helperPersonData.PlayerSteamID,
            freezemenu = helperPersonData.freezemenu,
                    ...
                    if (!ConVar.Find("mp_teammates_are_enemies")!.GetPrimitiveValue<bool>())
                    {
                        if (attackerteam != victimteam)
                        {
                            if (Configs.GetConfigData().KS_DefaultValue_HeadShotKillSound)
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(Configs.GetConfigData().KS_AddMenu_HeadShotKillSoundPath, 
                                            volume: personData.volume));
                                }
                            }
                            else
                            {
                    ...
                    if (!ConVar.Find("mp_teammates_are_enemies")!.GetPrimitiveValue<bool>())
                    {
                        if (attackerteam != victimteam)
                        {
                            if (Configs.GetConfigData().KS_DefaultValue_BodyKillSound)
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(Configs.GetConfigData().KS_AddMenu_BodyKillSoundPath, 
                                            volume: personData.volume));
                                }
                            }
                            else
    ...
    private HookResult OnPlayerHurt(EventPlayerHurt @event, GameEventInfo info)
    {
        ...
        if (attacker != victim)
        {
            if (hitgroup == 1) // Headshot hitgroup
            {
                    ...
                    if (!ConVar.Find("mp_teammates_are_enemies")!.GetPrimitiveValue<bool>())
                    {
                        if (attackerteam != victimteam)
                        {
                            if (Configs.GetConfigData().KS_DefaultValue_HeadShotHitSound)
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(Configs.GetConfigData().KS_AddMenu_HeadShotHitSoundPath, 
                                            volume: personData.volume));
                                }
                            }
                            else
                    ...
                    if (!ConVar.Find("mp_teammates_are_enemies")!.GetPrimitiveValue<bool>())
                    {
                        if (attackerteam != victimteam)
                        {
                            if (Configs.GetConfigData().KS_DefaultValue_BodyHitSound)
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(Configs.GetConfigData().KS_AddMenu_BodyHitSoundPath, 
                                            volume: personData.volume));
                                }
                            }
                            else
    ...
    private HookResult OnPlayerDeathQuake(EventPlayerDeath @event, GameEventInfo info)
    {
        ...
        if (KilledHimSelf)
        {
            // Move personData declaration here since it's needed in this block
            ...
            try
            {
                ...
                if (json.ContainsKey("SelfKill"))
                {
                    ...
                    if (!json["SelfKill"].ContainsKey("ShowCenter_InSecs")) SIntervalHUD = 0; 
                    else float.TryParse(json["SelfKill"]["ShowCenter_InSecs"]?.ToString(), out SIntervalHUD);
                    if (!json["SelfKill"].ContainsKey("Announcement")) Sanouncement = false; 
                    else bool.TryParse(json["SelfKill"]["Announcement"]?.ToString(), out Sanouncement);
                    if (!json["SelfKill"].ContainsKey("ShowChat")) SShowChat = false; 
                    else bool.TryParse(json["SelfKill"]["ShowChat"]?.ToString(), out SShowChat);
                    if (!json["SelfKill"].ContainsKey("ShowCenter")) SShowCenter = false; 
                    else bool.TryParse(json["SelfKill"]["ShowCenter"]?.ToString(), out SShowCenter);
                }
                ...
                if (!string.IsNullOrEmpty(SsoundPath))
                {
                    if (Sanouncement)
                    {
                        ...
                        allplayers.ForEach(players => 
                        {
                            ...
                            {
                                var playeridPlayer = players.SteamID;
                                var helperPersonDataA = Helper.RetrievePersonDataById(playeridPlayer);
                                KillSoundGoldKingZ.PersonData personDataA = new KillSoundGoldKingZ.PersonData
                                {
                                    PlayerSteamID = helperPersonDataA.PlayerSteamID,
                                    freezemenu = helperPersonDataA.freezemenu,
                                    headshotkill = helperPersonDataA.headshotkill,
                                    headshothit = helperPersonDataA.headshothit,
                                    bodyshotkill = helperPersonDataA.bodyshotkill,
                                    bodyshothit = helperPersonDataA.bodyshothit,
                                    quakesounds = helperPersonDataA.quakesounds,
                                    quakehmessages = helperPersonDataA.quakehmessages,
                                    quakecmessages = helperPersonDataA.quakecmessages,
                                    date = DateTime.Now,
                                    volume = 1.0f
                                };
                                ...
                                {
                                    Server.NextFrame(() => 
                                        players.EmitSound(SsoundPath, volume: personDataA.volume));
                                }
                    ...
                    else
                    {
                        ...
                        {
                            Server.NextFrame(() => 
                                attacker.EmitSound(SsoundPath, volume: personData.volume));
                        }
        ...
        var helperPersonDataAttacker = Helper.RetrievePersonDataById(playeridattacker);
        KillSoundGoldKingZ.PersonData personDataAttacker = new KillSoundGoldKingZ.PersonData
        {
            PlayerSteamID = helperPersonDataAttacker.PlayerSteamID,
            freezemenu = helperPersonDataAttacker.freezemenu,
            headshotkill = helperPersonDataAttacker.headshotkill,
            headshothit = helperPersonDataAttacker.headshothit,
            bodyshotkill = helperPersonDataAttacker.bodyshotkill,
            bodyshothit = helperPersonDataAttacker.bodyshothit,
            quakesounds = helperPersonDataAttacker.quakesounds,
            quakehmessages = helperPersonDataAttacker.quakehmessages,
            quakecmessages = helperPersonDataAttacker.quakecmessages,
            date = DateTime.Now,
            volume = 1.0f
        };
        ...
        if (attacker == victim)
        {
                ...
                if (!string.IsNullOrEmpty(SsoundPath))
                {
                    if (Sanouncement)
                    {
                        ...
                        allplayers.ForEach(players => 
                        {
                            if (players != null && players.IsValid && !players.IsBot)
                            {
                                var helperPersonDataa = Helper.RetrievePersonDataById(players.SteamID);
                                KillSoundGoldKingZ.PersonData personDataa = new KillSoundGoldKingZ.PersonData
                                {
                                    PlayerSteamID = helperPersonDataa.PlayerSteamID,
                                    freezemenu = helperPersonDataa.freezemenu,
                    ...
                    {
                        if (personDataAttacker.quakesounds)
                        {
                            // Skip
                        }
                        else
                        {
                            Server.NextFrame(() => 
                                attacker.EmitSound(SsoundPath, volume: personDataAttacker.volume));
                        }

                        if (personDataAttacker.quakecmessages)
                        {
                            // Skip
                        }
                        else
                        ...
                        if (personDataAttacker.quakehmessages)
                        {
                            // Skip
                        }
                        else
            ...
            if (Globals.Kill_Streak.ContainsKey(playeridattacker) || Globals.Kill_StreakHS.ContainsKey(playeridattacker) || 
                Globals.Kill_Knife.ContainsKey(playeridattacker) || Globals.Kill_Nade.ContainsKey(playeridattacker) || 
                Globals.Kill_Molly.ContainsKey(playeridattacker) || Globals.Kill_Taser.ContainsKey(playeridattacker))
            {
                try
                {
                    string _json = Path.Combine(ModuleDirectory, "../../plugins/Kill-Sound-GoldKingZ/config/Kill_Settings.json");
                    ...
                    if (json.ContainsKey("TeamKill"))
                    {
                        ...
                        if (!json["TeamKill"].ContainsKey("ShowCenter_InSecs")) TIntervalHUD = 0; 
                        else float.TryParse(json["TeamKill"]["ShowCenter_InSecs"]?.ToString(), out TIntervalHUD);
                        if (!json["TeamKill"].ContainsKey("Announcement")) Tanouncement = false; 
                        else bool.TryParse(json["TeamKill"]["Announcement"]?.ToString(), out Tanouncement);
                        if (!json["TeamKill"].ContainsKey("ShowChat")) TShowChat = false; 
                        else bool.TryParse(json["TeamKill"]["ShowChat"]?.ToString(), out TShowChat);
                        if (!json["TeamKill"].ContainsKey("ShowCenter")) TShowCenter = false; 
                        else bool.TryParse(json["TeamKill"]["ShowCenter"]?.ToString(), out TShowCenter);
                    }

                    if (!string.IsNullOrEmpty(TsoundPath))
                    {
                        if (attackerteam == victimteam && !ConVar.Find("mp_teammates_are_enemies")!.GetPrimitiveValue<bool>())
                        {
                            if (Tanouncement)
                            {
                                var allplayers = Helper.GetAllController();
                                allplayers.ForEach(players => 
                                {
                                    if (players != null && players.IsValid && !players.IsBot)
                                    {
                                        ...
                                        {
                                            Server.NextFrame(() => 
                                                players.EmitSound(TsoundPath, volume: personDataa.volume));
                                        }
                            ...
                            else
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(TsoundPath, volume: personDataAttacker.volume));
                                }
                    ...
                    if (attackerteam != victimteam || ConVar.Find("mp_teammates_are_enemies")!.GetPrimitiveValue<bool>() && attackerteam == victimteam) 
                        Globals.Kill_Streak[playeridattacker]++;
                    ...
                    if (json.ContainsKey("FirstBlood"))
                    {
                        ...
                        if (!json["FirstBlood"].ContainsKey("ShowCenter_InSecs")) FIntervalHUD = 0; 
                        else float.TryParse(json["FirstBlood"]["ShowCenter_InSecs"]?.ToString(), out FIntervalHUD);
                        if (!json["FirstBlood"].ContainsKey("Announcement")) Fanouncement = false; 
                        else bool.TryParse(json["FirstBlood"]["Announcement"]?.ToString(), out Fanouncement);
                        if (!json["FirstBlood"].ContainsKey("ShowChat")) FShowChat = false; 
                        else bool.TryParse(json["FirstBlood"]["ShowChat"]?.ToString(), out FShowChat);
                        if (!json["FirstBlood"].ContainsKey("ShowCenter")) FShowCenter = false; 
                        else bool.TryParse(json["FirstBlood"]["ShowCenter"]?.ToString(), out FShowCenter);
                    }
                    ...
                    if (Globals.First_Blood && !string.IsNullOrEmpty(FsoundPath))
                    {
                        if (Fanouncement)
                        {
                            ...
                            allplayers.ForEach(players => 
                            {
                                if (players != null && players.IsValid && !players.IsBot)
                                {
                                    ...
                                    {
                                        Server.NextFrame(() => 
                                            players.EmitSound(FsoundPath, volume: personDataa.volume));
                                    }
                        ...
                        else
                        {
                            ...
                            {
                                Server.NextFrame(() => 
                                    attacker.EmitSound(FsoundPath, volume: personDataAttacker.volume));
                            }
                    ...
                    if (json.ContainsKey("KnifeKill"))
                    {
                        ...
                        if (!json["KnifeKill"].ContainsKey("Interval_InSecs")) NInterval = 5; 
                        else double.TryParse(json["KnifeKill"]["Interval_InSecs"]?.ToString(), out NInterval);
                        if (!json["KnifeKill"].ContainsKey("ShowCenter_InSecs")) NIntervalHUD = 0; 
                        else float.TryParse(json["KnifeKill"]["ShowCenter_InSecs"]?.ToString(), out NIntervalHUD);
                        if (!json["KnifeKill"].ContainsKey("Announcement")) Nanouncement = false; 
                        else bool.TryParse(json["KnifeKill"]["Announcement"]?.ToString(), out Nanouncement);
                        if (!json["KnifeKill"].ContainsKey("ShowChat")) NShowChat = false; 
                        else bool.TryParse(json["KnifeKill"]["ShowChat"]?.ToString(), out NShowChat);
                        if (!json["KnifeKill"].ContainsKey("ShowCenter")) NShowCenter = false; 
                        else bool.TryParse(json["KnifeKill"]["ShowCenter"]?.ToString(), out NShowCenter);
                    }
                    else if (json.ContainsKey("KnifeKill_" + numberofknifekill))
                    {
                        NSteak = true;
                        NsoundPath = json["KnifeKill_" + numberofknifekill]["Path"]?.ToString() ?? "";
                        if (!json["KnifeKill_" + numberofknifekill].ContainsKey("Interval_InSecs")) NInterval = 5; 
                        else double.TryParse(json["KnifeKill_" + numberofknifekill]["Interval_InSecs"]?.ToString(), out NInterval);
                        if (!json["KnifeKill_" + numberofknifekill].ContainsKey("ShowCenter_InSecs")) NIntervalHUD = 0; 
                        else float.TryParse(json["KnifeKill_" + numberofknifekill]["ShowCenter_InSecs"]?.ToString(), out NIntervalHUD);
                        if (!json["KnifeKill_" + numberofknifekill].ContainsKey("Announcement")) Nanouncement = false; 
                        else bool.TryParse(json["KnifeKill_" + numberofknifekill]["Announcement"]?.ToString(), out Nanouncement);
                        if (!json["KnifeKill_" + numberofknifekill].ContainsKey("ShowChat")) NShowChat = false; 
                        else bool.TryParse(json["KnifeKill_" + numberofknifekill]["ShowChat"]?.ToString(), out NShowChat);
                        if (!json["KnifeKill_" + numberofknifekill].ContainsKey("ShowCenter")) NShowCenter = false; 
                        else bool.TryParse(json["KnifeKill_" + numberofknifekill]["ShowCenter"]?.ToString(), out NShowCenter);
                    }

                    if (knifekill && !string.IsNullOrEmpty(NsoundPath) && NInterval > 0)
                    {
                        if (!Globals.lastPlayTimesKnife.ContainsKey(playeridattacker) || 
                            (DateTime.Now - Globals.lastPlayTimesKnife[playeridattacker]).TotalSeconds >= NInterval)
                        {
                            if (Nanouncement)
                            {
                                var allplayers = Helper.GetAllController();
                                allplayers.ForEach(players => 
                                {
                                    if (players != null && players.IsValid && !players.IsBot)
                                    {
                                        ...
                                        {
                                            Server.NextFrame(() => 
                                                players.EmitSound(NsoundPath, volume: personDataa.volume));
                                        }
                            ...
                            else
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(NsoundPath, volume: personDataAttacker.volume));
                                }
                    ...
                    if (json.ContainsKey("GrenadeKill"))
                    {
                        ...
                        if (!json["GrenadeKill"].ContainsKey("Interval_InSecs")) GInterval = 5; 
                        else double.TryParse(json["GrenadeKill"]["Interval_InSecs"]?.ToString(), out GInterval);
                        if (!json["GrenadeKill"].ContainsKey("ShowCenter_InSecs")) GIntervalHUD = 0; 
                        else float.TryParse(json["GrenadeKill"]["ShowCenter_InSecs"]?.ToString(), out GIntervalHUD);
                        if (!json["GrenadeKill"].ContainsKey("Announcement")) Ganouncement = false; 
                        else bool.TryParse(json["GrenadeKill"]["Announcement"]?.ToString(), out Ganouncement);
                        if (!json["GrenadeKill"].ContainsKey("ShowChat")) GShowChat = false; 
                        else bool.TryParse(json["GrenadeKill"]["ShowChat"]?.ToString(), out GShowChat);
                        if (!json["GrenadeKill"].ContainsKey("ShowCenter")) GShowCenter = false; 
                        else bool.TryParse(json["GrenadeKill"]["ShowCenter"]?.ToString(), out GShowCenter);
                    }
                    else if (json.ContainsKey("GrenadeKill_" + numberofnadekill))
                    {
                        GSteak = true;
                        GsoundPath = json["GrenadeKill_" + numberofnadekill]["Path"]?.ToString() ?? "";
                        if (!json["GrenadeKill_" + numberofnadekill].ContainsKey("Interval_InSecs")) GInterval = 5; 
                        else double.TryParse(json["GrenadeKill_" + numberofnadekill]["Interval_InSecs"]?.ToString(), out GInterval);
                        if (!json["GrenadeKill_" + numberofnadekill].ContainsKey("ShowCenter_InSecs")) GIntervalHUD = 0; 
                        else float.TryParse(json["GrenadeKill_" + numberofnadekill]["ShowCenter_InSecs"]?.ToString(), out GIntervalHUD);
                        if (!json["GrenadeKill_" + numberofnadekill].ContainsKey("Announcement")) Ganouncement = false; 
                        else bool.TryParse(json["GrenadeKill_" + numberofnadekill]["Announcement"]?.ToString(), out Ganouncement);
                        if (!json["GrenadeKill_" + numberofnadekill].ContainsKey("ShowChat")) GShowChat = false; 
                        else bool.TryParse(json["GrenadeKill_" + numberofnadekill]["ShowChat"]?.ToString(), out GShowChat);
                        if (!json["GrenadeKill_" + numberofnadekill].ContainsKey("ShowCenter")) GShowCenter = false; 
                        else bool.TryParse(json["GrenadeKill_" + numberofnadekill]["ShowCenter"]?.ToString(), out GShowCenter);
                    }

                    if (NadeKill && !string.IsNullOrEmpty(GsoundPath) && GInterval > 0)
                    {
                        if (!Globals.lastPlayTimesNade.ContainsKey(playeridattacker) || 
                            (DateTime.Now - Globals.lastPlayTimesNade[playeridattacker]).TotalSeconds >= GInterval)
                        {
                            if (Ganouncement)
                            {
                                var allplayers = Helper.GetAllController();
                                allplayers.ForEach(players => 
                                {
                                    if (players != null && players.IsValid && !players.IsBot)
                                    {
                                        ...
                                        {
                                            Server.NextFrame(() => 
                                                players.EmitSound(GsoundPath, volume: personDataa.volume));
                                        }
                            ...
                            else
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(GsoundPath, volume: personDataAttacker.volume));
                                }
                    ...
                    if (json.ContainsKey("MollyKill"))
                    {
                        ...
                        if (!json["MollyKill"].ContainsKey("Interval_InSecs")) MInterval = 5; 
                        else double.TryParse(json["MollyKill"]["Interval_InSecs"]?.ToString(), out MInterval);
                        if (!json["MollyKill"].ContainsKey("ShowCenter_InSecs")) MIntervalHUD = 0; 
                        else float.TryParse(json["MollyKill"]["ShowCenter_InSecs"]?.ToString(), out MIntervalHUD);
                        if (!json["MollyKill"].ContainsKey("Announcement")) Manouncement = false; 
                        else bool.TryParse(json["MollyKill"]["Announcement"]?.ToString(), out Manouncement);
                        if (!json["MollyKill"].ContainsKey("ShowChat")) MShowChat = false; 
                        else bool.TryParse(json["MollyKill"]["ShowChat"]?.ToString(), out MShowChat);
                        if (!json["MollyKill"].ContainsKey("ShowCenter")) MShowCenter = false; 
                        else bool.TryParse(json["MollyKill"]["ShowCenter"]?.ToString(), out MShowCenter);
                    }
                    else if (json.ContainsKey("MollyKill_" + numberofmollykill))
                    {
                        MSteak = true;
                        MsoundPath = json["MollyKill_" + numberofmollykill]["Path"]?.ToString() ?? "";
                        if (!json["MollyKill_" + numberofmollykill].ContainsKey("Interval_InSecs")) MInterval = 5; 
                        else double.TryParse(json["MollyKill_" + numberofmollykill]["Interval_InSecs"]?.ToString(), out MInterval);
                        if (!json["MollyKill_" + numberofmollykill].ContainsKey("ShowCenter_InSecs")) MIntervalHUD = 0; 
                        else float.TryParse(json["MollyKill_" + numberofmollykill]["ShowCenter_InSecs"]?.ToString(), out MIntervalHUD);
                        if (!json["MollyKill_" + numberofmollykill].ContainsKey("Announcement")) Manouncement = false; 
                        else bool.TryParse(json["MollyKill_" + numberofmollykill]["Announcement"]?.ToString(), out Manouncement);
                        if (!json["MollyKill_" + numberofmollykill].ContainsKey("ShowChat")) MShowChat = false; 
                        else bool.TryParse(json["MollyKill_" + numberofmollykill]["ShowChat"]?.ToString(), out MShowChat);
                        if (!json["MollyKill_" + numberofmollykill].ContainsKey("ShowCenter")) MShowCenter = false; 
                        else bool.TryParse(json["MollyKill_" + numberofmollykill]["ShowCenter"]?.ToString(), out MShowCenter);
                    }

                    if (MollyKill && !string.IsNullOrEmpty(MsoundPath) && MInterval > 0)
                    {
                        if (!Globals.lastPlayTimesMolly.ContainsKey(playeridattacker) || 
                            (DateTime.Now - Globals.lastPlayTimesMolly[playeridattacker]).TotalSeconds >= MInterval)
                        {
                            if (Manouncement)
                            {
                                var allplayers = Helper.GetAllController();
                                allplayers.ForEach(players => 
                                {
                                    if (players != null && players.IsValid && !players.IsBot)
                                    {
                                        ...
                                        {
                                            Server.NextFrame(() => 
                                                players.EmitSound(MsoundPath, volume: personDataa.volume));
                                        }
                            ...
                            else
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(MsoundPath, volume: personDataAttacker.volume));
                                }
                    ...
                    if (json.ContainsKey("TaserKill"))
                    {
                        ...
                        if (!json["TaserKill"].ContainsKey("Interval_InSecs")) ZInterval = 5; 
                        else double.TryParse(json["TaserKill"]["Interval_InSecs"]?.ToString(), out ZInterval);
                        if (!json["TaserKill"].ContainsKey("ShowCenter_InSecs")) ZIntervalHUD = 0; 
                        else float.TryParse(json["TaserKill"]["ShowCenter_InSecs"]?.ToString(), out ZIntervalHUD);
                        if (!json["TaserKill"].ContainsKey("Announcement")) Zanouncement = false; 
                        else bool.TryParse(json["TaserKill"]["Announcement"]?.ToString(), out Zanouncement);
                        if (!json["TaserKill"].ContainsKey("ShowChat")) ZShowChat = false; 
                        else bool.TryParse(json["TaserKill"]["ShowChat"]?.ToString(), out ZShowChat);
                        if (!json["TaserKill"].ContainsKey("ShowCenter")) ZShowCenter = false; 
                        else bool.TryParse(json["TaserKill"]["ShowCenter"]?.ToString(), out ZShowCenter);
                    }
                    else if (json.ContainsKey("TaserKill_" + numberoftaserkill))
                    {
                        ZSteak = true;
                        ZsoundPath = json["TaserKill_" + numberoftaserkill]["Path"]?.ToString() ?? "";
                        if (!json["TaserKill_" + numberoftaserkill].ContainsKey("Interval_InSecs")) ZInterval = 5; 
                        else double.TryParse(json["TaserKill_" + numberoftaserkill]["Interval_InSecs"]?.ToString(), out ZInterval);
                        if (!json["TaserKill_" + numberoftaserkill].ContainsKey("ShowCenter_InSecs")) ZIntervalHUD = 0; 
                        else float.TryParse(json["TaserKill_" + numberoftaserkill]["ShowCenter_InSecs"]?.ToString(), out ZIntervalHUD);
                        if (!json["TaserKill_" + numberoftaserkill].ContainsKey("Announcement")) Zanouncement = false; 
                        else bool.TryParse(json["TaserKill_" + numberoftaserkill]["Announcement"]?.ToString(), out Zanouncement);
                        if (!json["TaserKill_" + numberoftaserkill].ContainsKey("ShowChat")) ZShowChat = false; 
                        else bool.TryParse(json["TaserKill_" + numberoftaserkill]["ShowChat"]?.ToString(), out ZShowChat);
                        if (!json["TaserKill_" + numberoftaserkill].ContainsKey("ShowCenter")) ZShowCenter = false; 
                        else bool.TryParse(json["TaserKill_" + numberoftaserkill]["ShowCenter"]?.ToString(), out ZShowCenter);
                    }

                    if (TaserKill && !string.IsNullOrEmpty(ZsoundPath) && ZInterval > 0)
                    {
                        if (!Globals.lastPlayTimesTaser.ContainsKey(playeridattacker) || 
                            (DateTime.Now - Globals.lastPlayTimesTaser[playeridattacker]).TotalSeconds >= ZInterval)
                        {
                            if (Zanouncement)
                            {
                                var allplayers = Helper.GetAllController();
                                allplayers.ForEach(players => 
                                {
                                    if (players != null && players.IsValid && !players.IsBot)
                                    {
                                        ...
                                        {
                                            Server.NextFrame(() => 
                                                players.EmitSound(ZsoundPath, volume: personDataa.volume));
                                        }
                            ...
                            else
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(ZsoundPath, volume: personDataAttacker.volume));
                                }
                    ...
                    if (json.ContainsKey("HeadShot"))
                    {
                        ...
                        if (!json["HeadShot"].ContainsKey("Interval_InSecs")) HInterval = 5; 
                        else double.TryParse(json["HeadShot"]["Interval_InSecs"]?.ToString(), out HInterval);
                        if (!json["HeadShot"].ContainsKey("ShowCenter_InSecs")) HIntervalHUD = 0; 
                        else float.TryParse(json["HeadShot"]["ShowCenter_InSecs"]?.ToString(), out HIntervalHUD);
                        if (!json["HeadShot"].ContainsKey("Announcement")) Hanouncement = false; 
                        else bool.TryParse(json["HeadShot"]["Announcement"]?.ToString(), out Hanouncement);
                        if (!json["HeadShot"].ContainsKey("ShowChat")) HShowChat = false; 
                        else bool.TryParse(json["HeadShot"]["ShowChat"]?.ToString(), out HShowChat);
                        if (!json["HeadShot"].ContainsKey("ShowCenter")) HShowCenter = false; 
                        else bool.TryParse(json["HeadShot"]["ShowCenter"]?.ToString(), out HShowCenter);
                    }
                    else if (json.ContainsKey("HeadShot_" + numberofkillsHS))
                    {
                        HSteak = true;
                        HsoundPath = json["HeadShot_" + numberofkillsHS]["Path"]?.ToString() ?? "";
                        if (!json["HeadShot_" + numberofkillsHS].ContainsKey("Interval_InSecs")) HInterval = 5; 
                        else double.TryParse(json["HeadShot_" + numberofkillsHS]["Interval_InSecs"]?.ToString(), out HInterval);
                        if (!json["HeadShot_" + numberofkillsHS].ContainsKey("ShowCenter_InSecs")) HIntervalHUD = 0; 
                        else float.TryParse(json["HeadShot_" + numberofkillsHS]["ShowCenter_InSecs"]?.ToString(), out HIntervalHUD);
                        if (!json["HeadShot_" + numberofkillsHS].ContainsKey("Announcement")) Hanouncement = false; 
                        else bool.TryParse(json["HeadShot_" + numberofkillsHS]["Announcement"]?.ToString(), out Hanouncement);
                        if (!json["HeadShot_" + numberofkillsHS].ContainsKey("ShowChat")) HShowChat = false; 
                        else bool.TryParse(json["HeadShot_" + numberofkillsHS]["ShowChat"]?.ToString(), out HShowChat);
                        if (!json["HeadShot_" + numberofkillsHS].ContainsKey("ShowCenter")) HShowCenter = false; 
                        else bool.TryParse(json["HeadShot_" + numberofkillsHS]["ShowCenter"]?.ToString(), out HShowCenter);
                    }

                    if (headshot && !string.IsNullOrEmpty(HsoundPath) && HInterval > 0)
                    {
                        if (!Globals.lastPlayTimesHS.ContainsKey(playeridattacker) || 
                            (DateTime.Now - Globals.lastPlayTimesHS[playeridattacker]).TotalSeconds >= HInterval)
                        {
                            if (Hanouncement)
                            {
                                var allplayers = Helper.GetAllController();
                                allplayers.ForEach(players => 
                                {
                                    if (players != null && players.IsValid && !players.IsBot)
                                    {
                                        ...
                                        {
                                            Server.NextFrame(() => 
                                                players.EmitSound(HsoundPath, volume: personDataa.volume));
                                        }
                            ...
                            else
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(HsoundPath, volume: personDataAttacker.volume));
                                }
                    ...
                    if (json.ContainsKey("Kill"))
                    {
                        ...
                        if (!json["Kill"].ContainsKey("Interval_InSecs")) Interval = 5; 
                        else double.TryParse(json["Kill"]["Interval_InSecs"]?.ToString(), out Interval);
                        if (!json["Kill"].ContainsKey("ShowCenter_InSecs")) IntervalHUD = 0; 
                        else float.TryParse(json["Kill"]["ShowCenter_InSecs"]?.ToString(), out IntervalHUD);
                        if (!json["Kill"].ContainsKey("Announcement")) Kanouncement = false; 
                        else bool.TryParse(json["Kill"]["Announcement"]?.ToString(), out Kanouncement);
                        if (!json["Kill"].ContainsKey("ShowChat")) KShowChat = false; 
                        else bool.TryParse(json["Kill"]["ShowChat"]?.ToString(), out KShowChat);
                        if (!json["Kill"].ContainsKey("ShowCenter")) KShowCenter = false; 
                        else bool.TryParse(json["Kill"]["ShowCenter"]?.ToString(), out KShowCenter);
                    }
                    else if (json.ContainsKey("Kill_" + numberofkills))
                    {
                        KSteak = true;
                        soundPath = json["Kill_" + numberofkills]["Path"]?.ToString() ?? "";
                        if (!json["Kill_" + numberofkills].ContainsKey("Interval_InSecs")) Interval = 5; 
                        else double.TryParse(json["Kill_" + numberofkills]["Interval_InSecs"]?.ToString(), out Interval);
                        if (!json["Kill_" + numberofkills].ContainsKey("ShowCenter_InSecs")) IntervalHUD = 0; 
                        else float.TryParse(json["Kill_" + numberofkills]["ShowCenter_InSecs"]?.ToString(), out IntervalHUD);
                        if (!json["Kill_" + numberofkills].ContainsKey("Announcement")) Kanouncement = false; 
                        else bool.TryParse(json["Kill_" + numberofkills]["Announcement"]?.ToString(), out Kanouncement);
                        if (!json["Kill_" + numberofkills].ContainsKey("ShowChat")) KShowChat = false; 
                        else bool.TryParse(json["Kill_" + numberofkills]["ShowChat"]?.ToString(), out KShowChat);
                        if (!json["Kill_" + numberofkills].ContainsKey("ShowCenter")) KShowCenter = false; 
                        else bool.TryParse(json["Kill_" + numberofkills]["ShowCenter"]?.ToString(), out KShowCenter);
                    }

                    if (!string.IsNullOrEmpty(soundPath) && Interval > 0)
                    {
                        if (!Globals.lastPlayTimes.ContainsKey(playeridattacker) || 
                            (DateTime.Now - Globals.lastPlayTimes[playeridattacker]).TotalSeconds >= Interval)
                        {
                            if (Kanouncement)
                            {
                                var allplayers = Helper.GetAllController();
                                allplayers.ForEach(players => 
                                {
                                    if (players != null && players.IsValid && !players.IsBot)
                                    {
                                        ...
                                        {
                                            Server.NextFrame(() => 
                                                players.EmitSound(soundPath, volume: personDataa.volume));
                                        }
                            ...
                            else
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        attacker.EmitSound(soundPath, volume: personDataAttacker.volume));
                                }
    ...
    private HookResult OnRoundStart(EventRoundStart @event, GameEventInfo info)
    {
        ...
        try
        {
            ...
            if (json.ContainsKey("RoundPrepare"))
            {
                ...
                if (!json["RoundPrepare"].ContainsKey("ShowCenter_InSecs")) PIntervalHUD = 0; 
                else float.TryParse(json["RoundPrepare"]["ShowCenter_InSecs"]?.ToString(), out PIntervalHUD);
                if (!json["RoundPrepare"].ContainsKey("ShowChat")) PShowChat = false; 
                else bool.TryParse(json["RoundPrepare"]["ShowChat"]?.ToString(), out PShowChat);
                if (!json["RoundPrepare"].ContainsKey("ShowCenter")) PShowCenter = false; 
                else bool.TryParse(json["RoundPrepare"]["ShowCenter"]?.ToString(), out PShowCenter);
            }
            ...
            if (!string.IsNullOrEmpty(PsoundPath))
            {
                ...
                allplayers.ForEach(players => 
                {
                    if (players != null && players.IsValid && !players.IsBot)
                    {
                        ...
                        {
                            Server.NextFrame(() => 
                                players.EmitSound(PsoundPath, volume: personData.volume));
                        }
    ...
    public void OnTick()
    {
                    ...
                    if (!string.IsNullOrEmpty(RsoundPath))
                    {
                        ...
                        allplayers.ForEach(players => 
                        {
                            if (players != null && players.IsValid && !players.IsBot)
                            {
                                ...
                                {
                                    Server.NextFrame(() => 
                                        players.EmitSound(RsoundPath, volume: personData.volume));
                                }
        ...
        foreach (var player in Helper.GetAllController())
        {
            ...
            if (Globals.menuon.ContainsKey(playerid))
            {
                ...
                MenuSetupList.Add(Localizer["menu.item.volume"]);
                boolValuesList.Add(false);
                ...
                else if ((player.Buttons == PlayerButtons.Moveleft || player.Buttons == PlayerButtons.Moveright) && !Globals.buttonPressed[playerid] && ((PlayerFlags)player.Pawn.Value!.Flags & PlayerFlags.FL_ONGROUND) == PlayerFlags.FL_ONGROUND)
                {
                    ...
                    if (currentLineName == Localizer["menu.item.volume"])
                    {
                        if (player.Buttons == PlayerButtons.Moveleft)
                        {
                            personData.volume = Math.Max(0.0f, personData.volume - 0.1f);
                        }
                        else if (player.Buttons == PlayerButtons.Moveright)
                        {
                            personData.volume = Math.Min(1.0f, personData.volume + 0.1f);
                        }
                        
                        // Save the volume along with other settings
                        Helper.SaveToJsonFile(playerid, personData.freezemenu, personData.headshotkill, personData.headshothit, 
                            personData.bodyshotkill, personData.bodyshothit, personData.quakesounds, personData.quakehmessages, 
                            personData.quakecmessages, personDate, personData.volume);
                        
                        player.ExecuteClientCommand("play sounds/ui/item_sticker_select.vsnd_c");
                        Globals.buttonPressed[playerid] = true;
                    }
                    else if (currentLineName == Localizer["menu.item.freeze"])
                    {
                        ...
                        Helper.SaveToJsonFile(playerid, personData.freezemenu, personData.headshotkill, personData.headshothit, 
                            personData.bodyshotkill, personData.bodyshothit, personData.quakesounds, personData.quakehmessages, 
                            personData.quakecmessages, personDate);
                        player.ExecuteClientCommand("play sounds/ui/item_sticker_select.vsnd_c");
                        Globals.buttonPressed[playerid] = true;
                    }
                    ...
                    else if (currentLineName == Localizer["menu.item.quake.center.messages"])
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(Localizer["menu.item.quake.center.messages"]))
                            {
                ...
                if (Globals.currentIndexDict.ContainsKey(playerid))
                {
                    ...
                    for (int i = startIndex; i < startIndex + visibleOptions && i < MenuSetup.Length; i++)
                    {
                        ...
                        if (currentMenuOption == Localizer["menu.item.volume"])
                        {
                            string volumeText = $"<font color='yellow'>{personData.volume:F1}</font>";
                            string lineHtml = i == currentIndex 
                                ? $"<font color='orange'> {Imageleft} {currentMenuOption} : {volumeText} {ImageRight} </font><br>"
                                : $"<font color='white' class='fontSize-sm'>  {currentMenuOption} : {volumeText}  </font><br>";
                        ...
                        else
                        {
                            ...
                            string lineHtml = i == currentIndex 
                                ? $"<font color='orange'> {Imageleft} {currentMenuOption} : {statusText} {ImageRight} </font><br>"
                                : $"<font color='white' class='fontSize-sm'>  {currentMenuOption} : {statusText}  </font><br>";
                            builder.AppendLine(lineHtml);
                        }
                    }
